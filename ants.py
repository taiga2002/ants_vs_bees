    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if place.ant.is_container:
                if not place.ant.can_contain(self):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        """
        currently, places=self.place, 0
        while currently.is_hive == False:
            if self.min_range<=places<=self.max_range and currently.bees!=[]:
                return random_bee(currently.bees)
            else:
                currently=currently.entrance
                places+=1
        return None
